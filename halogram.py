...
try:
    ...
    if os.geteuid() != 0:
        print("\n{}ERROR: Program must be run with root privileges. Try again with sudo:\n\t{}$ sudo python3 halogram.py{}\n".format(RED, GREEN, END))
        os._exit(1)
except:
...
def shutdown():
    print('\n\n{}Program Shutting Down.'
          '\nOriginally made for the Halogen Competition!{}'.format(GREEN, END))
    os._exit(0)
...
try:
    ...
    print("\n{}If you still cannot resolve this error, please contact me directly {}{}{}".format(RED, BLUE, RED, GREEN, str(sys.exc_info()[1]), END))
    os._exit(1)
...
def heading():
    ...
    HALOGRAM AN ARP POISONER BY BYRON AKA SNNDMAA
    """ + END + BLUE +
    '\n' + '{}Discover and Deny Unauthorized IPs Access to your Internet{}'.format(YELLOW, RED, YELLOW, BLUE).center(98)
    )
...
def optionBanner():
    ...
    sleep(0.2)
    print('\t{}[{}1{}]{} Block ONE Device'.format(YELLOW, RED, YELLOW, WHITE))
    sleep(0.2)
    print('\t{}[{}2{}]{} Kick SOME Devices'.format(YELLOW, RED, YELLOW, WHITE))
    sleep(0.2)
    print('\t{}[{}3{}]{} Kick ALL Devices'.format(YELLOW, RED, YELLOW, WHITE))
    sleep(0.2)
...
def runDebug():
    ...
    print(
    "{}Starting debug... (Please report this crash on this repo){}".format(
        RED, END))
    try:
    ...
    try:
        ...
        if defaultInterfaceMac == "" or not defaultInterfaceMac:
            ...
            "\n{}ERROR: Default Interface MAC Address could not be obtained. Please enter MAC manually.{}\n".format(
                RED, END))
            header = ('{}Halogram{}> {}Enter MAC Address {}(MM:MM:MM:SS:SS:SS): '.format(BLUE, WHITE, RED, END))
            return (input(header))
        else:
...
def regenOnlineIPs():
    ...
    if not defaultGatewayMacSet and defaultGatewayMac == "":
        ...
        header = ("{}Halogram}> {}Enter your gateway's MAC Address {}(MM:MM:MM:SS:SS:SS): ".format(BLUE, WHITE, RED, END))
...
def scanNetwork():
    ...
    try:
        ...
        print("\n{}If you still cannot resolve this error, please submit an issue to this repo {}{}{}".format(RED, BLUE, RED, GREEN, str(sys.exc_info()[1]), END))
        os._exit(1)
    try:
        regenOnlineIPs()
    except KeyboardInterrupt:
        ...
        if len(hostsList) == 0 or len(hostsList) == 1:
            if len(hostsList) == 1:
                ...
                "\n{}{}WARNING: There are {}0 hosts{} up on you network.\n\tIt looks like something went wrong {}:/{}".format(
                    GREEN, RED, GREEN, RED, GREEN, END))
                print(
                "\n{}If you are experiencing this error multiple times, please submit an issue to this repo\n{}".format(
                    RED, BLUE, END))
                os._exit(1)
    ...
    if options.targets is None and options.scan is False:
        try:
            ...
            while True:
                ...
                header = ('{}Halogram{}> {}'.format(BLUE, WHITE, END))
                choice = input(header)
...
if __name__ == '__main__':
    ...
    version = '2.0'
    examples = ('\nExamples:\n'+
                '  sudo python3 halogram.py --target 192.168.1.10 \n'+
                '  sudo python3 halogram.py -t 192.168.1.5,192.168.1.10 -p 30\n'+
                '  sudo python3 halogram.py -s\n'+
                '  sudo python3 halogram.py (interactive mode)\n')

    parser = optparse.OptionParser(epilog=examples,
        usage='sudo python3 %prog [options]',
        prog='halogram.py', version=('halogram ' + version))
